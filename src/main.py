...
async def translate_excel_batch(
    ...
    try:
        ...
        # Limit concurrent translations to avoid OOM (each load_workbook uses ~50-100MB)
        max_concurrent = 2
        ...
        log.info(f"[TIMING] asyncio.gather ({len(languages)} langs, max_concurrent={max_concurrent}): {perf_counter() - t_gather:.3f}s")
